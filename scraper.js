...
const retrieveGoogleURLSforSearchTerm = async (searchTerm) => {
  ...
  while (!totalResults || start <= totalResults) {
    try {
...
const parseJobInformationWithRetry = async (bodyText, retries = 3) => {
  for (let i = 0; i < retries; i++) {
    try {
      return await parseJobInformation(bodyText);
    } catch (error) {
      console.error(
        `\nAttempt ${i + 1} to parse job information failed. Retrying...\n`
      );
    }
  }
  throw new Error(`Failed to parse job information after ${retries} attempts.`);
};
...
const processUrl = async (url) => {
  try {
    ...
    const parsedJobInfo = await parseJobInformationWithRetry(bodyText);
    let jobObject;
